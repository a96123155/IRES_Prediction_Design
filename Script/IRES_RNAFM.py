...
# from esm import FastaBatchedDataset, Alphabet
...
parser.add_argument('--prefix', type=str, default = 'IRES_RNAFM')
...
parser.add_argument("--cls_loss_weight", type=float, default=2, help="If is -1, calculate it based on the train labels")
...
parser.add_argument("--truncate_num", type=int, default=1024, help="Training batch size")
parser.add_argument("--mask_prob", type = float, default = 0.15, help = "0.15 or 0")
parser.add_argument("--batch_toks", type=int, default=4096, help="Training batch size")
...
class RNAFM_linear(nn.Module):
    ...
    def forward(self, tokens):
        ...
        embeddings = self.rnafm(tokens, [12])["representations"][12]
        embeddings = embeddings[:, ..., 1:-1, :]
        
        tokens = tokens[:, 1:-1]
        mask = (tokens != 1).float()  # 创建一个mask，非padding部分为1，padding部分为0
        x = torch.sum(embeddings * mask.unsqueeze(-1), dim=1) / torch.sum(mask, dim=1, keepdim=True)
#         print(torch.sum(mask, dim=1, keepdim=True), x.shape)
...
#         self.inp_len = inp_len
    ...
    def forward(self, tokens):
        ...
        x = rnafm["representations"][12]
        
        if args.avg_emb:
            x_o = x[:, ..., 1:-1, :]
            tokens = tokens[:, 1:-1]
            mask = (tokens != 1).float()  # 创建一个mask，非padding部分为1，padding部分为0
            x = torch.sum(x_o * mask.unsqueeze(-1), dim=1) / torch.sum(mask, dim=1, keepdim=True)            
            print(torch.sum(mask, dim=1, keepdim=True), x.shape)
...
#             x_o = x_o.unsqueeze(2)
        ...
        o_dropout = self.dropout3(o_relu)
        o = self.output(o_dropout)
#         print('+++', o.shape, rnafm['logits'].shape)
        return o, rnafm['logits']
    
def init_weights(m):
    if isinstance(m, nn.Linear):
        # Using Xavier uniform initialization for weights
        init.xavier_uniform_(m.weight)
        # You can also use He initialization with `init.kaiming_uniform_(m.weight)`
...
#             print(masked_toks.shape)
            logit, esm_logit = model(masked_toks)
#             print(toks[0], masked_strs[0], masked_toks[0])
#             print(logit.shape, esm_logit.shape)
...
#             print(loss_seq.item(), mlm_loss_seq.item())
...
for fold in range(args.folds):
    ...
    print(f'----Fold {fold}: ', len(train_data), len(test_data), label_weight)
    
    ##### Load Model
    model = RNAFM_linear().to(device)
    print(model)
    ...
    if args.random_init:
        print('====Random Initialize Param====')
        model.apply(init_weights)
    ##### FineTune?
    if not args.finetune_esm:
        for name, value in model.named_parameters():
            if args.finetune_sixth_layer_esm:
                if 'rnafm' in name and 'rnafm.layers.11' not in name:
    ...
    for epoch in trange(1, args.epochs+1):
        ...
        if args.local_rank == 0:
            if test_metrics['AUPR'].values[0] > best_aupr:
                ...
                torch.save(best_model_state, f"../models/{args.prefix}_best_model_fold{fold}.pt")
                epoch_without_improvement = 0
            else:
    ...
    if args.local_rank == 0:
        ...
        metrics_df.to_csv(f"../results/{filename}_metrics.csv")

        print('====Save y_pred====')
        train_result.to_csv(f'../results/{filename}_train_fold{fold}.csv', index = False)
        test_result.to_csv(f'../results/{filename}_test_fold{fold}.csv', index = False)
        ...
        plt.savefig(f'../results/{filename}.tif')
